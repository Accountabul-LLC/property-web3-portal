...
DROP POLICY IF EXISTS "Public can read active tiers" ON public.membership_tiers;
CREATE POLICY "Public can read active tiers"
  ON public.membership_tiers FOR SELECT
  USING (is_active = true);

DROP POLICY IF EXISTS "Admins can manage tiers" ON public.membership_tiers;
...
VALUES
  ('Starter', 'starter', 19.00, 190.00,
   ...
   'Deed fraud monitoring included', false, 1, 'Get Started'),
  ('Professional', 'professional', 49.00, 490.00,
   ...
   'Best value for small portfolios', true, 2, 'Get Started'),
  ('Portfolio', 'portfolio', 99.00, 990.00,
   ...
   'Everything unlimited', false, 3, 'Get Started')
