...
import { emailSchema, requiredTextSchema } from '@/lib/formValidation';
...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signup' && !isAdminTab) {
      const parsedFullName = requiredTextSchema('Full name', NAME_MAX).safeParse(fullName);
      if (!parsedFullName.success) {
        toast.error(parsedFullName.error.issues[0]?.message || 'Enter your full name');
        return;
      }
      if (accountType === 'business') {
        const parsedCompany = requiredTextSchema('Company name', COMPANY_MAX).safeParse(companyName);
        if (!parsedCompany.success) {
          toast.error(parsedCompany.error.issues[0]?.message || 'Enter your company name');
          return;
        }
      }
    }
