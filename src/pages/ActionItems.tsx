...
const GITHUB_OWNER = 'JibreelMuhammad';
const GITHUB_REPO = 'property-web3-portal';
...
export default function ActionItems() {
  ...
  const buildRichIssueBody = (item: ActionItemRow) => {
    const files = parseFiles(item.files_json);
    const sections: string[] = [
      `## Context`,
      `Generated from **${item.source_type}** session via AI Panel.`,
      ``,
      `## Problem`,
    ...
    if (files.length) {
      sections.push(`## Files Involved`, ...files.map(f => `- \`${f}\``), ``);
    }
    if (item.expected_outcome) {
      sections.push(`## Expected Outcome`, item.expected_outcome, ``);
    }
    sections.push(
      `## Acceptance Criteria`,
      ...
      ``,
      `---`,
      `*Priority: ${item.priority} · Source: ${item.source_type} · Generated by AI Panel*`,
    );
    return sections.join('\n');
  };

  const getLabelsForItem = (item: ActionItemRow): string[] => {
    const labels = ['ai-generated', 'action-item'];
    if (item.priority === 'HIGH') labels.push('high-priority');
    if (item.source_type === 'debate') labels.push('agent-debate');
    const files = parseFiles(item.files_json);
    const hasFrontend = files.some(f => f.startsWith('src/'));
    const hasBackend = files.some(f => f.startsWith('supabase/'));
  ...
  const pushToGithub = async (item: ActionItemRow) => {
    ...
    try {
      ...
      const title = `[${item.priority}] ${item.title}`;
      const body = buildRichIssueBody(item);
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/github-agent`, {
        ...
        body: JSON.stringify({
          action: 'create_issue', owner: GITHUB_OWNER, repo: GITHUB_REPO,
          title, body, labels,
        }),
      });
      ...
      toast.success(`Pushed as issue #${data.number}`);
    } catch { toast.error('Failed to push'); }
    finally { setBusyIds(prev => { const n = new Set(prev); n.delete(item.id); return n; }); }
  };

  // Completion check: query GitHub for issue status
  const [checking, setChecking] = useState(false);
  const checkCompletion = async () => {
    const syncedItems = items.filter(i => i.github_issue_number && i.status !== 'done');
    if (syncedItems.length === 0) { toast.info('No synced open items to check'); return; }
    ...
    try {
      ...
      for (const item of syncedItems) {
        try {
          const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/github-agent`, {
            ...
            headers: { 'Authorization': `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({
              action: 'get_issue', owner: GITHUB_OWNER, repo: GITHUB_REPO,
      ...
      if (completed > 0) toast.success(`${completed} item(s) marked done from GitHub`);
      else toast.info('No closed issues found');
    } catch { toast.error('Completion check failed'); }
